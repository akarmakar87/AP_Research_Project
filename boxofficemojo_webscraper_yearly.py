...
url = 'https://www.boxofficemojo.com/year/{year}/?ref_=bo_yl_table_4'
movie_url = 'https://www.boxofficemojo.com/{}'

# go year by year

# for each year, go through the list in order of Gross
...
def main():
    ...
    for year in range(2019,2022):    
        ...
        temp_ranks = dict((genre, []) for genre in list(box_office_df)[1:])

        results = requests.get(url.format(year=year), headers=headers)
        ...
        temp_ranks = dict((key,round(np.mean(vals),5)) if len(vals) != 0 else (key,0) for key,vals in temp_ranks.items()) | {'Year': year}
        box_office_df = box_office_df.append(temp_ranks,ignore_index=True)
        box_office_df.to_csv('box_office_dataset_yearly.csv', mode='a', header=False, index=False)
